...
public class Main {
	...
	public static void PreInit(FMLPreInitializationEvent event) {
		MinecraftForge.EVENT_BUS.register(Ref.clientProxy);
	}
	...
	public class RegistrationHandler {
		...
		float pitch = 1;
		String name = "minecraft:block.anvil.break";

		@SubscribeEvent
		public void static onGuiOpen(GuiOpenEvent event) {
				...
				if (playOn == 1 || playOn == 3) {
					...
					if (sound != null) {
						Minecraft.getMinecraft().getSoundHandler()
								.playSound(PositionedSoundRecord.getMasterRecord(sound, (float) pitch));
					} else {
					}
				}
			}
